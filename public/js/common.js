 ...
 *
 ...
 function upload_one_image(dialog_title, input_selector, extra_params, app) {
    open_upload_dialog(dialog_title, function (dialog, files) {
        //console.log(files[0]);
        $(input_selector).val(files[0].filepath);//设定隐藏文本框的值为上图片路径
        //获取ID名为 #thumb-preview,并修改图片路径属性属性
        $(input_selector + '-preview').attr('src', files[0].preview_url);//全路径
        $(input_selector + '-name').val(files[0].name); //输出原本的名字
    }, extra_params, 0, 'image', app);
}
 */
function open_upload_dialog(dialog_title,callback,extra_params,multi,filetype,app){
	multi = multi?1:0;//传入的是0，只能单传
	filetype = filetype?filetype:'image';//图片类型
	app = app?app:GV.APP;   //获取当前应用名
...
function upload_multi_image(dialog_title, container_selector, item_tpl_wrapper_id, extra_params, app) {
      open_upload_dialog(dialog_title, function (dialog, files) {
        var tpl = $('#' + item_tpl_wrapper_id).html();  //获取ID相符合的文本控件内容
        var html = '';
        //遍历多个文件信息
